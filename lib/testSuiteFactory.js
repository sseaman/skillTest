...
/** @private */
// NEED to be first to set eveyrthing up downstream
var log = require('winston-simple').setLevels({'all' : 'verbose'}).getLogger('TestSuiteFactory');
...
TestSuiteFactory.fromMap = function(map, testBuilder) {
	...
	for (var testIdx=0;testIdx<map.tests.length; testIdx++) {
		...
		log.verbose("Registered Test: '"+tests[testIdx].getName()+"'");
	}
